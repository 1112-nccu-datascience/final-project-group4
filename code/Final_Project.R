...
test_data <- read.csv(file = file.path("D:/vscode_R/final/test.csv"))  # 讀取測試資料集
...
fe_training <- dataset_forXGB[1:1460,]
fe_test <- dataset_forXGB[1461:2919,]

# 將訓練資料轉換成 DMatrix 格式
dtrain <- xgb.DMatrix(data = data.matrix(fe_training[, - which(names(fe_training) %in% c('SalePrice'))]), label = fe_training$SalePrice)
...
rmse <- sqrt(mean((fe_training$SalePrice - predictions)^2))
print(rmse)

# 將測試資料轉換成 DMatrix 格式
dtest <- xgb.DMatrix(data = data.matrix(fe_test[, - which(names(fe_test) %in% c('SalePrice'))]))

# 進行預測
xgboost_pred <- predict(model, newdata = dtest)

# 將 XGBoost 模型的預測結果生成最終提交的 CSV 文件
xgboost_submission <- data.frame(Id = test_data$Id, SalePrice = xgboost_pred)
write.csv(xgboost_submission, file = "xgboost_submission.csv", row.names = FALSE)


#Ensamble 
predictions <- 0.7 * lasso_pred + 0.3 * xgboost_pred
Ensamble <- data.frame(Id = test_data$Id, SalePrice = predictions)
write.csv(Ensamble, file = "Ensamble7030.csv", row.names = FALSE)
